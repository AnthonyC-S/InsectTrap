...
# 1. for an image of width and height being (w, h) pixels, resize image
#   to (w', h'), where w/h = w'/h' and w' x h' = 262144
# 2. resize network input size to (w', h')
# 3. pass the image to network and do inference
# 4. if inference speed is too slow for you, try to make w' x h'
#   smaller, which is defined with DEFAULT_INPUT_SIZE
#   (in object_detection.py or ObjectDetection.cs)
import math
...
class ObjectDetection(object):
    ...
    def _non_maximum_suppression(self, boxes, class_probs, max_detections):
        ...
        while len(selected_boxes) < max_detections:
            ...
            # Set the probability of overlapping predictions to zero, and
            # udpate max_probs and max_classes.
    ...
    def _extract_bb(self, prediction_output, anchors):
        ...
        # (x,y) in the network outputs is the center of the bounding
        #   box. Convert them to top-left.
        x = x - w / 2
        y = y - h / 2
    ...
    def _update_orientation(self, image):
        ...
        if hasattr(image, "_getexif"):
            ...
            if (exif is not None) and exif_orientation_tag in exif:
                ...
                print(
                    "Image has EXIF Orientation: {}".format(
                        str(orientation)))
                # orientation is 1 based, shift to zero based and
                # flip/transpose based on 0-based values
                orientation -= 1
                if orientation >= 4:
    ...
    def predict(self, preprocessed_inputs):
        ...
        Need to be implemented for each platforms.
            i.e. TensorFlow, CoreML, etc.
        """
    ...
    def postprocess(self, prediction_outputs):
        ...
        Args:
            prediction_outputs: Output from the object detection model.
                                (H x W x C)
